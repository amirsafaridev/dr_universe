    ...
    public function like_dislike(Request $request)
    {
        ...
        if ($post) {
            ...
            if ($like != null) {
                ...
                // ایجاد نوتیفیکیشن
                Notif::create([
                    'user_id' => $post->user_id,          // صاحب پست
                    'owner_id' => $user_id,               // کسی که لایک کرده
                    'action' => 'لایک',                  // نوع اکشن
                    'desc' => 'پست شما مورد پسند واقع شد توسط ' . auth()->user()->username, // توضیحات
                    'notifiable_type' => Post::class,     // نوع مدل
                    'notifiable_id' => $post_id,          // شناسه پست
                ]);
            }
...
//        dd($fileName,$media_type);
        ...
        return redirect()->route('home_page')->with('success', 'فایل با موفقیت آپلود شد.');
