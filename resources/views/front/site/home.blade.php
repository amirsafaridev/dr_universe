...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        try {
            const archiveResponse = axios.post('/live-stream/save-archive', {
            });
            console.log('لایو به عنوان پست ذخیره شد:', archiveResponse.data.message);
        } catch (archiveError) {
            console.error('خطا در ذخیره لایو به عنوان پست:', archiveError.response ? archiveError.response.data : archiveError.message);
        }

        const postSearchInput = document.getElementById('postUserSearch');
        ...
        postSearchInput.addEventListener('keyup', function () {
            ...
            if (query === '') {
                document.getElementById('postUserList').innerHTML = '<li class="list-group-item">نام کاربری را وارد کنید تا جستجو شود.</li>';
                return;
            }
            ...
            fetch('/search-users', {
                ...
                .then(data => {
                    const postUserList = document.getElementById('postUserList');
                    postUserList.innerHTML = ''; // پاک کردن لیست قبلی

                    if (data.users.length > 0) {
                        data.users.forEach(user => {
                            const profileImage = user.profile ? `/storage/profiles/${user.profile}` : `/images/default-avatar.png`;

                            postUserList.innerHTML += `
                            <li class="list-group-item d-flex align-items-center" style="direction: ltr;border: none !important;">
                                ...
                                <a href="#" class="send-post-link" data-user-id="${user.id}">${user.username}${user.blue_tick}</a>
                            </li>
                        `;
                        });

                        // افزودن listener برای لینک‌ها جدید
                        document.querySelectorAll('.send-post-link').forEach(link => {
                        ...
                        postUserList.innerHTML = '<li class="list-group-item">هیچ کاربری یافت نشد</li>';
                    }
                })
                .catch(error => {
        ...
        function loadFrontJsWithVersion() {
            let version = new Date().getTime();  // استفاده از timestamp برای ورژن‌بندی
            console.log(version);
            $("#post-container *").off()
            // حذف نسخه قبلی فایل front.js بدون توجه به پارامترهای ورژن‌بندی
            $('script[src^="/assets/js/front.js"]').remove();

            // لود نسخه جدید front.js با پارامتر ورژن جدید
            $('<script>')
                .attr('src', '/assets/js/front.js?v=' + version)
                .appendTo('head');
        }
    });
    ...
    <script>
        ...
        var current_user_username='{{$currentUser->username}}';
        var current_user_profile='{{url('storage/profiles/'.$currentUser->profile)}}';
        var user_profile='{{url('storage/profiles/')}}';
    ...
    <style>
        body.modal-open {
            overflow: auto !important; /* Reset overflow to auto */
    ...
    <div class="home-container scroll-play">
        <div class="header">
            <div class="icons">
                <a href="{{ url('conversations') }}" title="Direct Messages">
                    ...
                    <div class="sec2">
                        ...
                        @endif
                            ...
                            @if($post->media_type == "video")
                                <video oncontextmenu="return false;" playsinline preload="auto" autoplay  muted disablePictureInPicture id="video-tag-{{$post->id}}" data-src="{{url('storage/posts/'.$post->media."#t=1")}}">
                        ...
                        @if($post->media_type == "link")
                            <video oncontextmenu="return false;" playsinline preload="auto"  muted disablePictureInPicture id="video-tag-{{$post->id}}" data-src="{{$post->media}}">
...
{{--                                        </div>--}}
                                        <div class="modal-body">
                                            <div class="search-container">
                                                <input type="text" id="postUserSearch" class="form-control" placeholder="نام کاربر را جستجو کنید..." style="margin-bottom: 15px;">
                                            </div>

                                            <ul id="postUserList" class="list-group">
                    ...
                    <div class="comment-main" @if($post->comments->count() == 0) style="display: none;" @endif>
                        @foreach($post->two_comments as $comment)
                            ...
                            <div class="comment-1 comment">
                                ...
                                <span class="comment_username">{!! $comment->user->username_with_blue_tick !!}</span>
    ...
    margin: 0px 0 30px 0;">
        ...
        <div id="loading" style="display: none; text-align: center;padding-bottom: 80px">
            <img style="width: 50px;" src="{{asset('img/Animation-1726047798009.gif')}}" alt="Loading..." />
        </div>
    </div>
