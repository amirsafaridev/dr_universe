...
// تابع برای توقف پخش تمام ویدئوها
                ...
                $('#follow-btn').click(function() {
                    ...
                    var userId = '{{ $user->id }}'; // شناسه‌ی کاربری که قرار است فالو شود

                    if (button.hasClass('following-btn')) {
                        // اگر در حالت فالوینگ باشد، تأییدیه بپرسید
                        var confirmUnfollow = confirm("آیا مطمئن هستید که می‌خواهید این کاربر را آنفالو کنید؟");
                        if (!confirmUnfollow) {
                            return; // اگر کاربر تأیید نکرد، خروج از تابع
                        }
                    }
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if(response.status === 'followed') {
                                button.text('Following').addClass('following-btn');
                            } else if(response.status === 'unfollowed') {
                                button.text('Follow').removeClass('following-btn');
                            }
                            location.reload();

                        },
                        error: function(xhr) {
                            console.error(xhr.responseText); // برای ثبت جزئیات خطا
            ...
            // وقتی modal باز می‌شود، اطلاعات پست را تنظیم کنید
            modal.addEventListener('show.bs.modal', function (event) {
                const button = event.relatedTarget;
                selectedPostData.postLink = button.getAttribute('data-post-link');
                selectedPostData.mediaUrl = button.getAttribute('data-media-url');
                selectedPostData.mediaType = button.getAttribute('data-media-type');
                selectedPostData.mediaId = button.getAttribute('data-media-id');
            });

            // فقط یک بار event listener اضافه می‌کنیم
            document.querySelectorAll('.send-post-link').forEach(function (link) {
                link.addEventListener('click', function (e) {
                    e.preventDefault();
                    const userId = this.dataset.userId;

                    // درخواست AJAX برای ارسال لینک پست به دایرکت کاربر انتخاب شده
                    fetch(`/conversations/user/${userId}`, {
                        ...
                        body: JSON.stringify({
                            post_link: selectedPostData.postLink,
                            media: selectedPostData.mediaUrl,
                            media_type: selectedPostData.mediaType,
                            id: selectedPostData.mediaId
                        })
                    })
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                // بستن modal و حذف backdrop
                                $('#userListModal').modal('hide').on('hidden.bs.modal', function () {
                                    $('.modal-backdrop').remove();
                                    $('body').removeClass('modal-open');
                                    $('body').css('overflow', '');
                                });
                            } else {
                                console.error('Failed to send post link.');
                            }
                        })
    ...
    <style>
        ...
        .request-sent-btn
        {
            background-color: #e1e1e1; /* رنگ دکمه درخواست */
            color: black;
            width: 100%;
            margin: 20px 0;
            border: none;
    ...
    <div class="header" >
        ...
        <h4>
            ...
            {!! $user->username_with_blue_tick !!}

        </h4>

    </div>

    <div class="user-info-section" style="padding: 0 !important;">
        <div class="follower-info" style="align-items: baseline;margin-top: 15px;">
            ...
            <div class="user-list" id="followings_list">
                ...
                @foreach($followings_list as $item)
                    <p>
                        <img data-src="{{url('storage/profiles/'.$item->profile)}}" alt="user-image" style="width: 50px; border-radius: 50%; margin-left: 20px">
                        <a href="{{route("profile_page",["username"=>$item->username])}}">
                            {!! $item->username_with_blue_tick !!}
                        </a>
                    </p>
                @endforeach
            ...
            <div class="user-list" id="followers_list">
                ...
                @foreach($followers_list as $item)
                    <p>
                        <img src="{{url('storage/profiles/'.$item->profile)}}" alt="user-image" style="width: 50px; border-radius: 50%; margin-left: 20px">
                        <a href="{{route("profile_page",["username"=>$item->username])}}">
                            {!! $item->username_with_blue_tick !!}
                        </a>
                    </p>
...
{{--                <div class="direct-message-btn" style="margin: 20px;">--}}
{{--                    @if (Auth::id() !== $user->id)--}}
{{--                        <a href="{{ route('conversations.create_or_show', ['userId' => $user->id]) }}" class="message-btn">--}}
{{--                            <i class="fa fa-send" style="font-size: 24px;"></i> <!-- آیکون پیام -->--}}
{{--                        </a>--}}
{{--                    @endif--}}
{{--                </div>--}}
        ...
        <div class="image-id" style="flex-direction: row-reverse;">
            <div>
                <div class="image" style="    direction: ltr;">
                    <img src="{{url('storage/profiles/'.$user->profile)}}"  id="profile-image" alt="user-image" style="width: 70px;border-radius: 50%;opacity: 1;aspect-ratio: 1/1;object-fit: cover;">
                    @if($currentUser->id == $user->id)
                        <div class="edit" style="    top: 70px !important;">
                ...
                <div class="user-info" style="text-align: left">
                    <p style="font-size: 15px;margin-top: 1rem;">{{$user->name}}</p>
                    <div style="margin-bottom: 5px">{!! nl2br(e($user->description)) !!}</div>
    ...
    <div class="follow-button-container">
        @if (Auth::id() !== $user->id)
            <button id="follow-btn" class="{{ $isFollowing ? 'following-btn' : ($followRequestPending ? 'request-sent-btn' : 'follow-btn') }}">
                {{ $isFollowing ? 'Following' : ($followRequestPending ? 'Request Sent' : 'Follow') }}
            </button>
        @endif
    </div>
    ...
    @if(($user->account_type === 'private' && $isFollowing) || Auth::id() == $user->id || $user->account_type === 'public')
        @if($posts)
            ...
            @foreach($posts as $post)
                ...
                @if($post->media_type == "video")
                    <a href="{{route("profile_page",["username"=>$post->user->username,"post_id"=>$post->id]).'#post_'.$post->id}}" class="post-img">
                        <video oncontextmenu="return false;" class="user_posts_video" preload="auto"  disablePictureInPicture data-src="{{url('storage/posts/'.$post->media."#t=1")}}"
                               style="">
                ...
                @if($post->media_type == "link")
                        <a href="{{route("profile_page",["username"=>$post->user->username,"post_id"=>$post->id]).'#post_'.$post->id}}" class="post-img">
                            ...
                            <video oncontextmenu="return false;" class="user_posts_video" preload="auto"  disablePictureInPicture data-src="{{$post->media}}" style="">
                                <source data-src="{{url('storage/posts/'.$post->media)}}">
                    </video>
                            <img src="{{url('assets/img/play.png')}}" class="video-play-icon-grid">
                        </a>
        ...
        <div id="post-list" class="user-profile-section scroll-play" style="@if($post_id == "") display: none @endif">
            @foreach($posts as $post)
                <div style="padding-bottom: 50px;" class="post-detail-1 post-detail" id="post_{{$post->id}}" data-p="{{$post->id}}" data-viewed="false">
                    <div class="sec1">
                    ...
                    <div class="sec2">
                        ...
                        @endif
                        @if($post->media_type == "video")
                            <video oncontextmenu="return false;" playsinline preload="auto" autoplay  muted  disablePictureInPicture data-src="{{url('storage/posts/'.$post->media."#t=1")}}">
                                <source data-src="{{url('storage/posts/'.$post->media)}}">
                            </video>
                        ...
                        @endif
                            ...
                            @endif
                            @if($post->media_type == "link")
                                <video oncontextmenu="return false;" playsinline preload="auto" autoplay  muted  disablePictureInPicture id="video-tag-{{$post->id}}" data-src="{{$post->media}}">
                                    <source data-src="{{url('storage/posts/'.$post->media)}}">
                                </video>
                                <img src="{{url('assets/img/play.png')}}" class="video-play-icon" style="z-index: 0 !important;">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <ul class="list-group">
                                                ...
                                                @empty
                                                    <li class="list-group-item">یک کاربر را دنبال کنید</li>
                                                @endforelse
                                            </ul>
                                        </div>
                                    </div>
        ...
        <div id="post-saved" class="user-profile-section scroll-play" style="display: none">
            @foreach($post_save as $post)
                    <?php $post = App\Models\Post::find($post->savable_id) ;if (!$post) continue;?>
    ...
    @else
        <div class="container">
            <div class="row align-items-center my-5">
                <div class="col-auto">
                    <img src="{{ asset('img/Group 1471.svg') }}" alt="Profile Picture" class="img-fluid rounded-circle" style="width: 50px; height: 50px;">
                </div>
                <div class="col">
                    <h3 class="h5">این اکانت خصوصی است</h3>
                    <p class="mb-0">برای دیدن عکس و ویدئو های این اکانت درخواست فالو بدید.</p>
                </div>
            </div>
        </div>
    @endif
...
{{--                <img src="{{url('path/to/your/loader.gif')}}" alt="Loading..." />--}}
{{--            </div>--}}
{{--        @endif--}}
...
{{--        let page = 2;--}}
{{--        let isLoading = false;--}}
{{--        let currentView = @if($post_id != "") 'post-grid' @else 'post-list' @endif;--}}


{{--        $(document).on('click', '.profile_navbar_item', function(e) {--}}
{{--            e.preventDefault();--}}
{{--            let selectedView = $(this).data('selected');--}}

{{--            console.log(selectedView);--}}

{{--            // تنظیم نمای جاری--}}
{{--            currentView = selectedView;--}}
{{--            page = (selectedView === 'post-grid') ? 2 : 1;--}}

{{--            // پنهان کردن سایر نماها و نمایش نمای انتخاب شده--}}
{{--            $('#posts-grid, #post-list, #post-saved').hide();--}}
{{--            $('#' + selectedView).show() // نمایش نمای انتخاب شده و خالی کردن محتوا--}}


{{--            // if (selectedView === 'post-list') {--}}
{{--            //     $("#posts-grid").empty(); // خالی کردن محتوا--}}
{{--            // }--}}


{{--        loadMoreData(page, currentView); // لود کردن داده با نمای جدید--}}
...
{{--                    isLoading = true;--}}
{{--                    page++;--}}
{{--                    loadMoreData(page, currentView);--}}
{{--                }--}}
{{--            }--}}
{{--        });--}}

{{--        function loadMoreData(page, viewType) {--}}
{{--            console.log("test");--}}
...
{{--                url: '?page=' + page,--}}
{{--                type: "get",--}}
{{--                data: { view: viewType }, // ارسال نوع ویو--}}
{{--                beforeSend: function() {--}}
{{--                    $('#loading').show();--}}
{{--                }--}}
{{--            })--}}
...
{{--                    // افزودن محتوای جدید به نمای جاری--}}
{{--                    switch(viewType) {--}}
{{--                        case 'post-list':--}}
{{--                            $("#post-list").append(data.html);--}}
{{--                            break;--}}
{{--                        case 'post-saved':--}}
{{--                            $("#post-saved").append(data.html);--}}
{{--                            break;--}}
{{--                        case 'post-grid':--}}
{{--                        default:--}}
{{--                            $("#posts-grid").append(data.html);--}}
{{--                            break;--}}
{{--                    }--}}
